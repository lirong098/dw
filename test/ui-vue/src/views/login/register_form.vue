...
<script>
import { validateFormEmail, validateValue, validatePassword, validateConfirmPassword } from './public.js'
export default {
  name: '',
  components: {},
  props: {
  ...
  data () {
    return {
      ...
      registerRules: {
        ...
        phone: [
          {type: 'number', trigger: 'blur', message: '手机号码格式不正确'},
  ...
  methods: {
    ...
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$api.login.users(this.registerForm).then((res) => {
            console.log('注册', res)
            if (res.status === 201) {
              this.$message({
                message: '注册成功',
