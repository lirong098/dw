<template>
  <div class="test_detailed_contain">
    <div class="fl test_detailed_main main-card-box">
      ...
      <table-extend v-if="tableExtendShow"
                    :tableType="tableType"
  ...
  export default {
    ...
    methods: {
      ...
      saveBtn(addFun) {
        ...
        if (this.detaMessageForm.model.unit_meta.meta_id === "") {
          showModal.showModal(this, "", "", "单位不能为空，请填写！", '错误提示');
          return;
        }
        if (this.tableType === 1) {
...
//          }
      ...
      saveBtnConfirm(addFun) {
        if (this.tableType === 1) {
          this.detaMessageForm.model.extend.elastic_week_start.value = this.detaMessageForm.model.extend.elastic_week_start.value ?  COMMON.formatDate(this.detaMessageForm.model.extend.elastic_week_start.value) : null
          this.detaMessageForm.model.extend.elastic_week_end.value = this.detaMessageForm.model.extend.elastic_week_end.value ?  COMMON.formatDate(this.detaMessageForm.model.extend.elastic_week_end.value): null
          this.detaMessageForm.model.extend.delivery_week_first.value = this.detaMessageForm.model.extend.delivery_week_first.value ?  COMMON.formatDate(this.detaMessageForm.model.extend.delivery_week_first.value): null
          this.detaMessageForm.model.extend.delivery_week_last.value = this.detaMessageForm.model.extend.delivery_week_last.value ? COMMON.formatDate(this.detaMessageForm.model.extend.delivery_week_last.value) : null
        }
        this.detaMessageForm.item = this.detaMessageForm.item.filter(this.isDelete);
        let url = "/api/mrp_order/mrp_models/";
        let text = "保存成功";
        let type = "post";
        if (this.detaMessageForm.model.model_id > 0) {
      ...
      deleteMessageBtnConfirm() {
        console.log(console.log(this.detaMessageForm))
      ...
      * options,//post时数据*/
      ...
      //网络请求Item表体信息
      getRequestItem() {
        this.vmRequst('/api/mrp_order/mrp_items/?model_id=' + this.detaMessageForm.model.model_id, function (that, objRes) {
      ...
      updateExtend(extend = false) {
        ...
        }else if (this.tableType !== 1 && (this.rowId === 0 || !this.rowId)) {
          return extendIntialData;
        }else if (this.tableType !== 1 && this.rowId > 0 && extend) {
          let modelExtend = ['spec']
          modelExtend.forEach(res => {
            if (!this.detaMessageForm.model.extend[res]) {
              this.detaMessageForm.model.extend[res] = ''
            }
...
//        });
    ...
    mounted() {
      ...
      if (this.detaMessageForm.model.model_id == 0 || this.detaMessageForm.model.model_id == " " || this.detaMessageForm.model.model_id == null) {
        ...
        this.$set(this.$data, 'tableExtendShow', true);
