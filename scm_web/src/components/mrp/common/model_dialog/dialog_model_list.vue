      ...
      * status 1 请求列表 2模糊查询(code) 4 delete*/
      ...
      vmRequstApi(qty, page, type, customer_id, season_meta_id) {
        ...
        if (this.modelName && this.modelName != '') {
          url = api + '/api/mrp_order/mrp_models/?search=' + this.modelName + '&exclude=' + this.exclude;
        } else {
          url = api + '/api/mrp_order/mrp_models/?page=' + page + '&pagesize=' + qty + '&exclude=' + this.exclude;
        }
