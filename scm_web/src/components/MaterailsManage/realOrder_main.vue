  ...
  export default{
    ...
    methods: {
      getOrderData: function () {
        ...
        let geturl = api+'/api/fcstpr/fact_orders/?pr_order_type='+ this.currentOrderType+'&page='
      ...
      editOrder: function (index, row) {
        ...
        '__MC_FACT_ORDER_IMPLANTATION' ? 1 :
          row.pr_type_meta.meta_code ==
          '__MC_FACT_ORDER_REPLENISHMENT' ? 2 : '';
      ...
      deleteOrder: function (type,index, row) {
        const api = this.$store.state.DEV_API;
        if(type == 1){//单个
          fetchAPI('delete',api + '/api/fcstpr/fact_order/?fcst_pr_id='+row.fcst_pr_id,null,this).then((res) => {
            if(res.status == 204){
              this.$message({
                message: '删除成功',
                type: 'success'
          ...
          fetchAPI('delete',api + '/api/fcstpr/fact_orders/',{data:code},this).then((res) => {
            if(res.status == 204){
              this.$message({
                message: '删除成功',
                type: 'success'
