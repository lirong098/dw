        ...
        <el-tabs v-model="currentTab" type="card"
          ...
          >
            <div v-if="item.name == 'main'">
              <orderInputMain @changeTabs="changeTabs"
                              :editData="editData"
                              :isFresh="isFresh"
                              @cancelTab="cancelTab"></orderInputMain>
            </div>
            <div
              v-else-if="addOrderData[item.index] && item.name == 'addOrder_'+addOrderData[item.index].tabIndex">
              <orderInputEdit @changeTabs="changeTabs"
                              @cancelTab="cancelTab"
                              @isUpdate="isUpdate"
                              :customerData="customerData"
                              :currentTab="currentTab"
                              :editOrder="addOrderData[item.index]"
                              :optType="item.type"></orderInputEdit>
            </div>
            <div
            ...
            <div
              v-else-if="viewOrderData[item.index] &&
            item.name ==
            'viewOrder_'+viewOrderData[item.index].tabIndex">
              <orderInputView :customerData="customerData"
                              :order="viewOrderData[item.index]"
                              :optType="item.type"></orderInputView>
            </div>
...
<script>
  import orderInputMain from '../../components/ForecaseOrder/orderInput_main.vue';
  import orderInputEdit from '../../components/ForecaseOrder/orderInput_edit.vue';
  import orderInputView from '../../components/ForecaseOrder/orderInput_view.vue';
        ...
        if (data) {
          if (data.type == 'addOrder') {
            ...
            for (let tab in this.TABS) {
              if (this.TABS[tab].index == data.data.fcst_order_id) {
                isAddTab = false;
                break
              }
            }
            if (isAddTab) {
              data.data.tabIndex = data.data.fcst_order_id;
              this.editOrderData[data.data.fcst_order_id] = Object.assign({}, data.data);
              let tab = {
                title: '编辑手工订单',
                name: data.type + '_' + data.data.fcst_order_id,
                index: data.data.fcst_order_id,
                type: data.type,
                closable: true
              };
              this.currentTab = tab.name;
              ...
              this.currentTab = data.type + '_' + data.data.fcst_order_id;
            }
...
//            this.getPrsData();
            ...
            if(isAddTab){
              data.data.tabIndex = data.data.fcst_order_id;
              this.viewOrderData[data.data.fcst_order_id] = Object.assign({}, data.data);
              let tab = {
                title:'查看Model',
                name:data.type+'_'+data.data.tabIndex,
                index:data.data.tabIndex,
      ...
      removeTab: function (targetName,parent_currentTab) {
        for (let tab in this.TABS) {
          if (this.TABS[tab].name == targetName) {
            ...
            } else if (targetName.indexOf('editOrder_')
              != -1) {
            ...
            }else if (targetName.indexOf('viewOrder_') != -1) {
              delete this.viewOrderData[this.TABS[tab].index]
            }else if
