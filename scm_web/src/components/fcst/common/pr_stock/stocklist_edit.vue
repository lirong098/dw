  ...
  <div class="test_detailed_contain">
    <div class="fl test_detailed_main main-card-box">
          <div class="clearfix" style="position:relative;">
            <div  class="dw-badge">
              <h4 :class="billState.className">{{billState.value}}</h4>
            </div>
            <div class="card_box">
                  ...
                  <el-form class="detaMessageForm" :model="detaMessageForm" ref="detaMessageForm">
                    ...
                    <el-col :span="12">
                      <el-form-item label="备料交期：">
                        <el-date-picker v-model="detaMessageForm.fcst_pr.delivery_date"
                                        type="week"
                                        format="yyyy-MM-dd (WW周)" placeholder="选择日期"
                        ...
                        <!--<dw-date-picker v-model="detaMessageForm.fcst_pr.delivery_date"-->
                                        <!--mode="range"-->
                                        <!--placeholder="选择日期"-->
                                        <!--v-show="changeStatus"-->
                                        <!--style="width:60%">-->
                    ...
                    <div style="width: 99%;">
                      <el-col :span="12">
                        <el-form-item label="单据日期：">
                          <el-input class="nochange"
                                    v-text="detaMessageForm.fcst_pr.deal_date ?detaMessageForm.fcst_pr.deal_date:''"
                                    v-show="!changeStatus"></el-input>
                          <el-date-picker v-model="detaMessageForm.fcst_pr.deal_date"
                                          type="date" placeholder="选择日期"
                                          :picker-options="pickerOptions1"
                                          @change="changeDealDate"
                                          v-show="changeStatus">
                          </el-date-picker>
                          <!--<dw-date-picker v-model="detaMessageForm.fcst_pr.deal_date"-->
                                          <!--mode="single"-->
                                          <!--placeholder="选择日期"-->
                                          <!--v-show="changeStatus"-->
                                          <!--style="width:60%">-->
                    ...
                    <el-col :span="12">
                      <el-form-item label="材料类型：">
                        <el-input class="nochange"
                                  v-text="typeReturn(detaMessageForm.fcst_pr.spec.material_type_meta_id)" v-show="!changeStatus"></el-input>
                        <el-select v-model="detaMessageForm.fcst_pr.spec.material_type_meta_id" placeholder="请选择" v-show="changeStatus" :disabled="fcstType !== 8">
                          <el-option
                            @change="selectChange()"
                            v-for="item in metaDate"
                            :key="item.meta_id"
            ...
            <el-card>
              <el-col class="composition">
                <el-table ref="multipleTable"
                  ...
                  <el-table-column label="款号" width="100">
                    <template scope="scope">
                      <span>{{scope.row.item.model.extend ? scope.row.item.model.extend.iman_code ? scope.row.item.model.extend.iman_code.value: '' : ''}}</span>
                    </template>
                  </el-table-column>
                  <el-table-column label="颜色" width="100">
                    <template scope="scope">
                      <span>{{scope.row.item.model.spec ? scope.row.item.model.spec.color:''}}</span>
                    </template>
                  </el-table-column>

                  <el-table-column label="Item" show-overflow-tooltip v-if="detaMessageForm.fcst_pr.spec.material_type_meta_id === 50">
                    <template scope="scope">
                      <el-input v-show="!changeStatus"
                  ...
                  <el-table-column label="数量" width="100">
                    <template scope="scope">
                      <span v-show="!changeStatus">{{scope.row.quantity === 0 ? '' : scope.row.quantity}}</span>
                      <el-input v-show="changeStatus" v-model="scope.row.quantity"></el-input>

                    </template>
                  </el-table-column>
                  <el-table-column label="单位" prop="item.model.unit_meta.meta_name" width="80"></el-table-column>
    ...
    <div class=" click-button">
      <el-card class="extra_card" v-if="changeStatus === false && (detaMessageForm.fcst_pr.fcst_pr_id)">
        <el-button type="primary" size="large" @click="addData" v-if="fcstType ===8">新增</el-button>
        <el-button type="danger" size="large" @click="changesMethod">修改</el-button>
        <el-button type="success" size="large" style="font-size: 14px; padding: 11px 0px;" @click="postMRP">发送MRP
        </el-button>
        <el-dropdown @command="handleCommand">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item command="searchDate">备料比率</el-dropdown-item>
      ...
      <el-card class="extra_card" v-if="changeStatus !== false || !(detaMessageForm.fcst_pr.fcst_pr_id)">
        ...
        <el-button type="success" size="danger" @click="addDataSaveBtn" v-if="fcstType ===8">
          <p>保存</p>
          <p>新增</p>
        </el-button>
    ...
    <!--点击查看备料比率后的弹窗-->
    <el-dialog  style="float: left" :visible.sync="modelTypeSave" v-if="modelTypeSave" size="large">
      <stock-list
        :fcstPrId="detaMessageForm.fcst_pr.fcst_pr_id"
        v-on:closeDialog="closeDialog" :fcstType="this.fcstType">
      </stock-list>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  import stockList from "./stock_diolag.vue";
  //公共文件
  ...
  export default {
      ...
      return {
        ...
        detaMessageForm: {
          fcst_pr: {
            ...
            spec: {
              material_type_meta_id:this.propData.length > 0 ? this.propData['0'].material_type_meta_id ? this.propData['0'].material_type_meta_id: 49:49
            },
            material_type_meta:{
              meta_name:''
            }
        ...
        pickerOptions1: { //选择日期数据
          firstDayOfWeek:1,
        },
        onlyGetItem: false, //model弹窗的参数 true时
        noGetItem:true,
    ...
    methods: {
      ...
      // 发送按钮 lxn START
      postMRP() {
        if (this.detaMessageForm.fcst_pr.is_send_mrp) {
          showModal.showModal(this, this.postMrpConfirm, "", '此单据 "已发送MRP" , 是否继续发送?', '操作提示', "是", "否");
        } else {
          this.postMrpConfirm()
        }
      },
      postMrpConfirm () {
        fetchAPI('post', this.$store.state.NEW_API + '/api/newfcst/fcstorder2mrporder/', this.detaMessageForm, this).then((res) => {
      ...
      selectChange() {
        ...
        fetchAPI('get', api + '/api/mat_models/metas/?parent_meta_code=48', null, this).then((res) => {
          if (res && res.status === 200) {
            this.metaDate = res.data;
          }
        })
      },
      //处理备料交期、实单交期信息
      getDeliveryDate() {
        if (this.propData.length > 0) {
          if (this.propData['0'].delivery_date) {
            return this.propData['0'].delivery_date
          } else {
      ...
      // 保存按钮
      saveBtn(addFun) {
        if (this.detaMessageForm.fcst_pr.fcst_pr_no === " "
          || this.detaMessageForm.fcst_pr.fcst_pr_no === null
          || !this.detaMessageForm.fcst_pr.fcst_pr_no) {
          showModal.showModal(this, "", "", "名称不能为空，请填写！", '错误提示');
          return;
        }

        for (let i in this.detaMessageForm.fcst_pr_item) {
          if(this.detaMessageForm.fcst_pr_item[i].item.model.model_id === 0
            && this.detaMessageForm.fcst_pr_item[i].item.model.model_code === '') {
            showModal.showMessage(this, "error", "请完成表格！");
            return;
          }
        }
      ...
      saveBtnConfirm(addFun) {
        ...
        const api = this.$store.state.NEW_API;
        this.detaMessageForm.fcst_pr.delivery_date = COMMON.formatDate(this.detaMessageForm.fcst_pr.delivery_date)
        fetchAPI(type, api + url, this.detaMessageForm, this).then((objRes) => {
          this.detaMessageForm = objRes.data;
          this.tableRowIndex = "";
          this.updateItem(this.detaMessageForm.fcst_pr_item);
          this.ifItemCount();
          this.changeStatus = false;
      ...
      getModelInfo(obj, model_id) {
        ...
        if (obj.model_item === "" || !obj.model_item) {
          const api = this.$store.state.NEW_API;
          fetchAPI('get', api + '/api/mat_models/get_free_size/?model_id=' + obj.model_id, null, this).then((res) => {
            if (res && res.status === 200) {
              this.detaMessageForm.fcst_pr_item[this.iconIndex].item = res.data
              this.detaMessageForm.fcst_pr_item[this.iconIndex].item.model = obj
            }
          })
...
//          };
      ...
      selectModel(index, row) {
        ...
        if(this.detaMessageForm.fcst_pr.spec.material_type_meta_id === 49) {
        ...
        if(this.detaMessageForm.fcst_pr.spec.material_type_meta_id === 50) {
          this.$refs.multipleTable.setCurrentRow(row);//高亮当前行
      ...
      updateItem(resData) {
        ...
        for (let x in resData) {
          if(!resData[x].item.model.spec){
            resData[x].item.model.spec = {}
            resData[x].item.model.spec.color = ''
          }
          if(!resData[x].item.model.extend){
            resData[x].item.model.extend = {}
            resData[x].item.model.extend.iman_code = {}
            resData[x].item.model.extend.iman_code.value = ''
          }
      ...
      getRequstModel() {
        requst.requst(this, "/api/mat_models/fcst_pr/?fcst_pr_id=" + this.detaMessageForm.fcst_pr.fcst_pr_id, function (that, objRes) {
          ...
          if(objRes.data.fcst_pr.is_send_mrp){
            that.billState = {
              className: 'dw-badge-text',
              value: '已发送MRP'
            }
...
//        this.detaMessageForm.fcst_pr.delivery_date = valArray['0']
//      },
      changeDealDate(val) {
        this.detaMessageForm.fcst_pr.deal_date = val
      },
      //操作更多下拉菜单触发事件
    ...
    updated() {
      if(this.detaMessageForm.fcst_pr.spec.material_type_meta_id === 50) {
        let divname = 'table';
        this.keydown(3, divname);
      }else if(this.detaMessageForm.fcst_pr.spec.material_type_meta_id === 49){
        let divname = 'table';
...
//      this.keydown(2,divname);
    ...
    computed: {
      deliveryDate:function () {
        return this.detaMessageForm.fcst_pr.delivery_date ? this.detaMessageForm.fcst_pr.delivery_date+' ('+COMMON.getWeekNumber(this.detaMessageForm.fcst_pr.delivery_date)+'周)':''
      }
    },
    mounted() {
      this.getStockType();
