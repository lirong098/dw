                ...
                <div style="width: 99%">
                  <el-col :span="12">
                    <el-form-item label="季节：">
                      <el-input class="truechange nochange" v-text="sampleData.season?sampleData.season.meta_name:''" disabled ></el-input>
                    </el-form-item>
                  </el-col>
                </div>
                <el-col :span="12">
          ...
          <div class="table">
            <el-card>
              <el-table :data="sampleItemsTableData" border style="width: 100%">
                ...
                <el-table-column label="Model名称" width="300">
                  <template scope="scope">
                    <!--<el-input v-show="checkStatus === 'look'"-->
      ...
      <el-card class="extra_card" v-show="checkStatus === 'look'">
        ...
        <el-dropdown>
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-if="fcstType==2"><a @click="addPreviewTab(sampleData.fcst_order_id)">生成备料单</a></el-dropdown-item>
            <el-dropdown-item><a @click="deleteMessageBtn()" style="display: inline-block; width: 100%;">删除</a></el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </el-card>
  ...
  export default {
    ...
    methods: {
      ...
      getSample() {
        ...
        fetchAPI('get', api+'/api/newfcst/fcst_order_item/?fcst_order_id=' + this.childRowId, {}, this).then((res) => {
          if (res && res.status === 200) {
            ...
            console.log(this.sampleItemsTableData)
          }
        });
      },
      // 修改分页条数
      handleSizeChange: function (val) {
        this.pagesize = val;
        const api =  this.$store.state.NEW_API;
        fetchAPI('get', api+'/api/newfcst/fcst_order_item/?fcst_order_id=' + this.childRowId + '&pagesize=' + this.pagesize + '&page=' + this.currentPage, {}, this).then((res) => {
          if (res && res.status === 200) {
            this.sampleItemsTableData = res.data.data;
            this.pagedatacount = res.data.total_number
            console.log(this.sampleItemsTableData)
      ...
      // 备料单预览
      addPreviewTab(fcstOrderId) {
        let newTabName = ++this.chlidTabIndex+100 + '';
        let addChildTABS = {
          title: "备料单预览",
          name: newTabName,
          closable: true,
